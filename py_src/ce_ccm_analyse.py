...
def permutation_test(x, y, E=3, tau=1, num_perm=100):
    ...
    for _ in range(num_perm):
        ...
        rho_perm.append(ccm.ce_ccm(x_perm, y, E=E, tau=tau))
...
def process_stock(stock_file):
    ...
    x_features = ['open_qfq', 'high_qfq', 'low_qfq', 'pct_chg', 'vol', 'volume_ratio', 'pe_ttm', 'ps_ttm', 'ema_qfq_5',
                  'ema_qfq_10', 'ema_qfq_20', 'ema_qfq_30', 'ema_qfq_60', 'kdj_qfq', 'kdj_d_qfq', 'kdj_k_qfq',
                  'rsi_qfq_12', 'rsi_qfq_24', 'rsi_qfq_6', 'macd_qfq', 'macd_dea_qfq', 'macd_dif_qfq']
    y_feature = 'close_qfq'
    ...
    for feature in x_features:
        ...
        f_to_t_causality = ccm.ce_ccm(feature_series, target_series, E=3, tau=1)
        causality_results[f'{feature} -> {y_feature}'] = f_to_t_causality

        # Calculate causality from target to feature (y_feature -> feature)
        t_to_f_causality = ccm.ce_ccm(target_series, feature_series, E=3, tau=1)
...
def main():
    # 配置参数
    data_dir = 'ablation_exp'  # 股票数据存放目录
