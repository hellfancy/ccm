...
def main():
    ...
    data_dir = os.path.join(script_dir, 'ablation_exp')  # 股票数据存放目录
    results_dir = 'CE_CCM_Final_Analysis'  # 结果保存目录
    ...
    # 使用多进程处理股票
    with Pool() as pool:
        results = pool.map(process_stock, stock_files)

    all_results = {os.path.splitext(os.path.basename(f))[0]: r for f, r in zip(stock_files, results) if r is not None}
    ...
    save_and_plot(df_x_to_y.T, 'CE-CCM Analysis: Features to Target (Close Price)', 'features_to_target')

    # Plot for Y -> X (Rows: Stocks, Cols: Features)
    save_and_plot(df_y_to_x.T, 'CE-CCM Analysis: Target (Close Price) to Features', 'target_to_features')
